...
$relativeModelPath = $modelPath
$relativeOutputPath = $OutputPath
try {
    if (Test-Path $modelPath) {
        $relativeModelPath = Resolve-Path -LiteralPath $modelPath -Relative
    }
    $relativeOutputPath = Join-Path 'models' (Split-Path -Path $OutputPath -Leaf)
} catch {
...
try {
    ...
    if ($tool) {
        ...
        $env:MODEL_PATH = $relativeModelPath
        $env:OUTPUT_PATH = $relativeOutputPath
        $env:QTYPE = $QType
...
env_threads = os.environ.get("LLAMA_QUANT_NTHREAD")
...
if env_threads:
    try:
        params.nthread = max(int(env_threads), 1)
    except Exception:
...
'@ | python - 2>&1 | Tee-Object -FilePath $logPath -Append
        $exitCode = $LASTEXITCODE
        Write-Log ("Python quantize exited with code {0}" -f $exitCode)
    }
}
catch {
    Write-Log $_
